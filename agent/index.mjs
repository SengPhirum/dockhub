#!/usr/bin/env node
// knetrahub-agent (formerly dockhub-agent) dispatcher. Deployed as a
...
// as before (DOCKHUB_AGENT_MODE is accepted too, as an alias, for the same
...
const modes = (process.env.KNETRAHUB_AGENT_MODE || process.env.DOCKHUB_AGENT_MODE || 'docker')
  .split(',')
  .map((m) => m.trim())
  .filter(Boolean)
